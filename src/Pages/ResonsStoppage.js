...
// const ReasonsStoppage = () => { // Corrected the component name
...
//       try {
//         const response = await axios.get(`http://localhost:5000/get_save_off_reasons`);
...
//     // If endDate is not provided, assume it's the same as startDate
...
//     // If endDate is before startDate, assume it's the next day
//     if (parsedEndDate.isBefore(parsedStartDate)) {
//       effectiveEndDate = parsedStartDate.add(1, 'day').format('D/M/YY');
//       console.warn(`End date was before start date. Assuming end date as next day: ${effectiveEndDate}`);
//     }
...
//     // Define the date-time format based on your data
...
//       const diffMinutes = dayjs.duration(end.diff(start)).asMinutes();
//       console.log(`Time Difference: ${diffMinutes} minutes`);
//       return diffMinutes;
//     }

//     console.error('Invalid date or time format:', { startDateTimeStr, endDateTimeStr });
//     return null; // Invalid date or time format
//   };

//   // Function to filter data based on selected date, filter type, and minimum time difference
//   const filterData = () => {
//     let temp = data;

//     // Apply Date Filter if a date is selected
//     if (filterDate) {
//       // Convert filterDate from 'YYYY-MM-DD' to 'D/M/YY'
//       const parsedDate = dayjs(filterDate).format('D/M/YY');
//       console.log(`Filtering by date: ${parsedDate}`);
//       temp = temp.filter(item => item.date === parsedDate);
//     }

//     // Apply Time Difference Filter if a minimum difference is set
//     if (minTimeDifference > 0) {
//       temp = temp.filter(item => {
//         if (item.endTime) {
//           const diff = calculateTimeDifference(item.date, item.time, item.endDate, item.endTime);
//           console.log(`Item ID: ${item._id}, Time Difference: ${diff}`);
...
//           }
//         }
//         return false; // Exclude entries without endTime
//       });
//       console.log(`Filtering by minimum time difference: ${minTimeDifference} minutes, Type: ${filterType}`);
//     }

//     setFilteredData(temp);
//     console.log(`Filtered Data Count: ${temp.length}`);
//   };
