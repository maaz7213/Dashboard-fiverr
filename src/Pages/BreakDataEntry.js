...
const BreakDataEntry = () => {
  ...
  // Filter out entries that already have saved reasons
  const filterUnsavedData = (data) => {
    return Object.keys(data).reduce((acc, machineKey) => {
      const filteredItems = data[machineKey].filter((item) => !isReasonSaved(item));
      if (filteredItems.length > 0) {
        acc[machineKey] = filteredItems;
      }
      return acc;
    }, {});
  };
  ...
  // Filter out the data that has already been saved
  const unsavedData = filterUnsavedData(groupedData);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      {/* Render grouped data by machine and channel */}
      {Object.keys(unsavedData).map((machineKey) => (
        <div key={machineKey}>
          <h3
            style={{ cursor: 'pointer' }}
            onClick={() => toggleMachineVisibility(machineKey)}
          ...
          {visibleMachines[machineKey] && (
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px' }}>
              ...
              <tbody>
                {unsavedData[machineKey].map((item) => (
                  <tr key={item._id}>
                    <td style={tableCellStyle}>{item.deviceno}</td>
                    <td style={tableCellStyle}>{item.channel}</td>
                    <td style={tableCellStyle}>{item.date}</td>
