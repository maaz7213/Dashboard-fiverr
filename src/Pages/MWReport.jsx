...
//       try {
//         const response = await axios.get('http://localhost:5000/shiftwise');
...
//       // console.log('Fetching data'); // CLG 1
//       try {
//         const response = await axios.get('http://localhost:5000/get_operator_selections');
//         // console.log('API Response:', response.data.message); // CLG 2
...
//           <h2>Date: {device.currentdate}</h2>
//           {/* Iterate over each channel */}
//           {Object.keys(device).filter(key => key.startsWith('ch')).map(channelKey => (
...
//                 const [runPercentage, averagePercentage] = [
//                   (shiftData.run_time / shiftData.shift_time) * 100 || 0,
//                   (shiftData.average / shiftData.average_threshold) * 100 || 0,
//                 ];
...
//                     />
//                     <p>Shift Duration: {shiftData.shift_time ? (typeof shiftData.shift_time === 'string' ? shiftData.shift_time : `${(shiftData.shift_time / 60).toFixed(2)} hours`) : 'N/A'}</p>
